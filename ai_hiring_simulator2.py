...
if st.button("Run 100-Round AI Interview") and jd_text and resume_text:
    ...
    def run_simulation_with_progress(jd_text, resume_text, num_rounds=5):
        ...
        for i in range(1, num_rounds + 1):
            try:
                logs.append(f"Round {i}: Generating question...")
                ...
                logs.append(f"Round {i}: Candidate answering...")
                status_container.markdown("\n".join(logs))
                ...
                logs.append(f"Answer: {answer}")
                status_container.markdown("\n".join(logs))

                logs.append(f"Round {i}: Evaluating answer...")
                status_container.markdown("\n".join(logs))
                ...
                logs.append(f"Round {i}: Interviewer deciding...")
                status_container.markdown("\n".join(logs))
                ...
                logs.append(f"Interviewer Decision: {'YES ✅' if interviewer_decision else 'NO ❌'}")
                status_container.markdown("\n".join(logs))
                interviewer_decisions.append(interviewer_decision)

                logs.append(f"Round {i}: Candidate deciding...")
                status_container.markdown("\n".join(logs))
                ...
                logs.append(f"Candidate Decision: {'YES ✅' if candidate_decision else 'NO ❌'}")
                status_container.markdown("\n".join(logs))
                ...
                logs.append(f"Error in round {i}: {str(e)}")
                status_container.markdown("\n".join(logs))
                time.sleep(1)
