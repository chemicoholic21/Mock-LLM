...
async def round_evaluation_agent(jd: str, resume: str, transcript: list):
    ...
    - Your justification should be concise and directly reference the transcript.
...
async def final_evaluation_agent(jd: str, resume: str, transcript: list):
    ...
    transcript_str = json.dumps(transcript, indent=2)
    prompt = f"""
    Act as the final decision-maker. Provide a final evaluation of the candidate's performance based on the *entire* interview.
    - Provide a final score from 1 (poor fit) to 100 (excellent fit).
    - The justification should summarize the candidate's overall performance.
    ...
    {transcript_str}
    ---
    Example: {{"score": 98, "justification": "An exceptional candidate. They consistently provided structured, data-driven answers that demonstrated deep technical expertise and a strong alignment with the company's goals. A clear hire."}}
    """
    return await model_generate_json(prompt, expected_type='object')
...
async def run_full_interview(jd: str, resume: str, num_conv_rounds: int, num_questions_per_round: int, progress_callback):
    """Orchestrates the entire interview, maximizing parallel API calls within each round."""
    full_transcript = []
    round_results = []
    total_steps = (num_conv_rounds * 3) + 1 # 3 steps per round + 1 final eval

    for i in range(num_conv_rounds):
        current_step_base = i * 3
    ...
    # Calculate final decisions based on round outcomes
    ...
    final_interviewer_decision = "YES" if num_conv_rounds > 0 and interviewer_yes_count / num_conv_rounds >= 0.5 else "NO"
